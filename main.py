...
NSFW_ALERT_CHANNEL_ID = 1352394388449267897
NSFW_ALERT_USER_ID = 968698192411652176

class NSFWView(View):
    def __init__(self, message_to_moderate):
        super().__init__(timeout=None)
        self.message_to_moderate = message_to_moderate

    @button(label="Удалить", style=discord.ButtonStyle.red)
    async def delete_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            await self.message_to_moderate.delete()
    ...
    @button(label="Замутить", style=discord.ButtonStyle.grey)
    async def mute_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        member = self.message_to_moderate.guild.get_member(self.message_to_moderate.author.id)
        if member:
            try:
                await member.edit(timeout=datetime.utcnow() + timedelta(minutes=30))
                await interaction.response.send_message("Пользователь замучен на 30 минут.", ephemeral=True)
            except:
                await interaction.response.send_message("Не удалось замутить.", ephemeral=True)

    @button(label="Пропустить", style=discord.ButtonStyle.green)
            ...
            if attachment.filename.lower().endswith(("jpg", "jpeg", "png", "gif", "webp")):
                try:
                    response = requests.post(
                        ...
                        files={'image': requests.get(attachment.url, stream=True).raw},
                    ...
                    if nsfw_score > 0.85:
                        alert_channel = message.guild.get_channel(NSFW_ALERT_CHANNEL_ID)
                        if alert_channel:
                            embed = Embed(
                                title="🚨 Обнаружен NSFW-контент",
                                description=f"Сообщение от {message.author.mention} может содержать NSFW.\n**Вероятность:** `{round(nsfw_score * 100, 2)}%`",
                                color=Color.red()
                            )
                            embed.set_image(url=attachment.url)
                            embed.set_footer(text=f"ID пользователя: {message.author.id}")
                            await alert_channel.send(
                                content=f"<@{NSFW_ALERT_USER_ID}>",
                                embed=embed,
                                view=NSFWView(message)
                            )
                except Exception as e:
