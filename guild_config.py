...
_CACHE_TTL = 60.0
_settings_locks: WeakKeyDictionary[asyncio.AbstractEventLoop, asyncio.Lock] = WeakKeyDictionary()


def _settings_lock() -> asyncio.Lock:
    loop = asyncio.get_running_loop()
    lock = _settings_locks.get(loop)
    if lock is None:
        lock = asyncio.Lock()
        _settings_locks[loop] = lock
    return lock
...
async def _load_effective_settings(guild_id: int) -> dict[str, Any]:
    merged = defaults()
    if guild_id != 0:
        merged.update(await _load_stored(0) or {})
    merged.update(await _load_stored(guild_id) or {})
    return merged
...
async def get_settings(guild_id: int) -> dict[str, Any]:
    ...
    if guild_id < 0:
        raise ValueError("guild_id must be non-negative")
    now = time.monotonic()
    ...
    async with _settings_lock():
        ...
        if cached is not None and now - _cache_ts.get(guild_id, 0.0) < _CACHE_TTL:
            return dict(cached)
        merged = await _load_effective_settings(guild_id)
        _cache[guild_id] = merged
        _cache_ts[guild_id] = time.monotonic()
        return dict(merged)
...
async def update_settings(guild_id: int, changes: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    ...
    async with _settings_lock():
        ...
        await set_guild_settings_raw(
            guild_id,
            json.dumps(updated_overrides, ensure_ascii=False, sort_keys=True),
        )
        if guild_id == 0:
            invalidate()
        else:
            invalidate(guild_id)
        updated = await _load_effective_settings(guild_id)
        _cache[guild_id] = updated
        _cache_ts[guild_id] = time.monotonic()
        return dict(updated), rejected
