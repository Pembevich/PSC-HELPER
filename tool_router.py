    ...
    return [
        {
            ...
            "content": (
                ...
                "или reference_context. Выбери не более восьми точных имён из available_tools. "
                "Это доступные возможности для многошаговой работы, а не обязательный список вызовов. "
                "Разрешай необходимые read/list-инструменты для поиска точной цели и проверки результата; "
                "следующие вызовы смогут использовать результаты предыдущих. Выбирай минимальный "
                "достаточный набор. Удаление или редактирование одного сообщения — manage_message, "
                "массовая очистка заданного количества — delete_messages. DISCORD_MESSAGE_REFERENCES "
                "содержит ID текущего сообщения, reply (включая reply на P.OS) и сообщения по ссылкам. "
                "'Удали это сообщение' с reply означает manage_message, не read_messages/search_logs. "
                "Без конкретной цели или способа её найти верни clarify. Строго различай создание "
