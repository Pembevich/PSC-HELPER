...
- `DB_BACKUP_CHANNEL_ID` — private Discord channel for gzip-compressed, integrity-checked SQLite backups (recommended on Railway). Copies above the attachment limit are split into verified parts; a final manifest marks a complete copy. Restore accepts older single-file copies and falls back to an earlier complete copy if the newest upload is damaged or incomplete.
